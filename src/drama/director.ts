...
export class Director {
  ...
  async openScene(seed: string, ctx?: DramaContext): Promise<Scene | null> {
    ...
    const baseRules = [
      ...
      "- 【名实相符铁律】名字/诨号若字面暗示某种生理或行为特征（如「哑」=不能说话、「瞎/盲」=看不见、「聋」=听不见、「瘸」=腿脚不便、「疯」「冷面」「毒手」等），" +
        "该人物的 identity/personality/style 必须与之【真正吻合】：起名「哑X」，其 style 就必须是不能说话、只以手势/书写/呜咽表意，绝不能又给他一套絮叨健谈的说话风格；" +
        "反之，若你想要一个健谈碎嘴的角色，就【不要】给他起一个字面意为‘哑/不能言’的名号。名字与人设自相矛盾即为废案，务必二者取一、彼此自洽。",
