...
export function buildCharacterSystem(c: Character, genrePersona = "武侠小说"): string {
  return [
    ...
    `- 你的台词与动作要【自洽、经得起推敲】：${SELF_CONSISTENCY_STANDARD}尤其：若你的身份/性格设定带生理限制（哑/盲/聋/重伤/失声），务必据此表演——哑者/失声者不说话，只以手势、书写、眼神、点头摇头、喉间呜咽与动作表意（括号写动作，不写成句台词），盲者不靠视觉、聋者不靠听觉，绝不违背设定。`,
